...
export class Pen {
    ...
    cleanSilhouette() {
        ...
        const d = src.data;
        const isOpaque = (x, y) => x >= 0 && y >= 0 && x < w && y < h && d[(x + y * w) * 4 + 3] > 0;
        for (let x = 0; x < w; x++) {
            for (let y = 0; y < h; y++) {
                const i = (x + y * w) * 4;
                let n = 0;
                ...
                if (d[i + 3] > 0) {
                    if (n <= 1)
                        out.data[i + 3] = 0; // orphan / lone tip → cut
                }
                else if (n >= 5) {
    ...
    snapToPalette() {
        ...
        for (let i = 0; i < d.length; i += 4) {
            if (d[i + 3] < 250)
                continue; // leave translucent FX (glow/sparkle) alone
            let best = 0;
    ...
    weather(amount) {
        ...
        const img = this.ctx.getImageData(ox, oy, w, h);
        const d = img.data;
        const opaque = (x, y) => x >= 0 && y >= 0 && x < w && y < h && d[(x + y * w) * 4 + 3] > 0;
        const interior = [];
        for (let x = 1; x < w - 1; x++) {
            for (let y = 1; y < h - 1; y++) {
    ...
    fillCone(cx, cy, dx, dy, startD, len, halfBase, light, dark) {
        ...
        for (let x = minX; x <= maxX; x++) {
            for (let y = minY; y <= maxY; y++) {
                ...
                if (Math.abs(nn) > 0.8)
                    shade -= 0.45 * ((Math.abs(nn) - 0.8) / 0.2);
     ...
     */
    drawRibbon(rx, ry, dx, dy, len, width, ramp, opts = {}) {
        ...
        for (let s = 0; s <= steps; s++) {
            ...
            for (let w = -hw; w <= hw; w += 0.5) {
                if (opts.swallowtail && t > 0.72 && Math.abs(w) < hw * 0.55)
    ...
    drawBladeHelper(params) {
        ...
        for (const p of bladeCorePoints) {
            ...
            if (st?.serrate != null && nd > 0.2 && nd < 1 - params.taperFactor) {
                ...
                if (side === "edge" || side === "both")
                    p.widthL += amt;
                if (side === "spine" || side === "both")
                    p.widthR += amt;
            }
        }
    ...
    drawCrossguardHelper(params) {
        ...
        const currentPoint = [start, new Vector(start)];
        const xguardControlPoints = [[], []];
        const xguardAngle = [(-Math.PI * 3) / 4, Math.PI / 4];
        const xguardOmega = [0, 0];
        const xguardOmegaCoolTimer = [0, 0];
    ...
    // -- round ornament --------------------------------------------------------
    drawRoundOrnamentHelper(params) {
        this.rng.checkpoint();
        const r = this.rng;
