...
const GREEN = { shadow: { r: 0x2f, g: 0x5a, b: 0x2e }, mid: { r: 0x4c, g: 0x87, b: 0x3a }, light: { r: 0x6f, g: 0xb0, b: 0x4a } };
const HEADS = [
    "orb", "orb", "crystal", "cluster", "crescent", "halo", "claws", "claws",
    "wings", "loop", "crook", "twinhorns", "star", "branch",
];
const SHAFTS = ["straight", "straight", "twisted", "wrapped", "spiral", "gnarled", "bone", "metal", "lacquer"];
const BINDINGS = [
    "none", "none", "collar", "wrap", "spiralcord", "leaves", "vines",
    "ribbons", "talisman", "feathers", "runes", "charm",
];
const FEET = ["none", "ferrule", "ferrule", "cap", "spike", "orb", "claw", "sphere"];
/** Legacy names from earlier generators, mapped so persisted configs don't break. */
const LEGACY_HEADS = { bare: "orb", collar: "orb" };
const LEGACY_SHAFTS = { segmented: "spiral" };
const LEGACY_BINDINGS = { doublecollar: "collar", rings: "wrap" };
...
export function drawStaff(pen, parts) {
    ...
    const head = requestedHead && HEADS.includes(requestedHead)
        ...
        : requestedHead && requestedHead in LEGACY_HEADS
            ...
            : pick(r, HEADS);
    const requestedShaft = parts?.shaft;
    const shaft = requestedShaft && SHAFTS.includes(requestedShaft)
        ? requestedShaft
        : requestedShaft && requestedShaft in LEGACY_SHAFTS
            ? LEGACY_SHAFTS[requestedShaft]
            : pick(r, SHAFTS);
    const requestedBinding = parts?.binding;
    const binding = requestedBinding && BINDINGS.includes(requestedBinding)
        ? requestedBinding
        : requestedBinding && requestedBinding in LEGACY_BINDINGS
            ? LEGACY_BINDINGS[requestedBinding]
            : pick(r, BINDINGS);
    const foot = parts?.foot && FEET.includes(parts.foot) ? parts.foot : pick(r, FEET);
...
function drawCollarBand(pen, bounds, diag, halfWidth, dscale, light, dark) {
...
function spiralShaft(pen, bounds, topDiag, haftR, dscale, base) {
...
function spiralCord(pen, bounds, startDiag, lenDiag, haftR, dscale) {
...
function vineWrap(pen, bounds, topDiag, haftR, dscale, r) {
    const perpX = Math.SQRT1_2, perpY = Math.SQRT1_2;
...
/** A flat rune-tablet hanging on a short cord below the head. */
function talisman(pen, bounds, gemOrtho, gemRadius, u, n, dscale, r, light, dark) {
    const rootDiag = (gemOrtho - gemRadius * 0.8) * Math.SQRT2;
    const rp = diagToPosition(rootDiag, bounds);
    const cordStr = colorStr(DARK.mid);
    ...
    for (let ox = -hw; ox <= hw; ox += 0.5) {
        for (let oy = -hh; oy <= hh; oy += 0.5) {
            if (Math.abs(ox) + Math.abs(oy) > hw + hh)
                continue;
...
/** Glowing rune glyphs carved down the shaft, in one crystal colour. */
function runeShaft(pen, bounds, topDiag, haftR, dscale, r) {
    const rune = pickCrystal(r);
...
function drawFoot(pen, bounds, foot, haftR, dscale, u, light, dark, r) {
    if (foot === "none")
        return;
