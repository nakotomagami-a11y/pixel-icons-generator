 ...
 */
function scaled(base, dx, y, dscale, lt, seed) {
 ...
 */
function leather(base, dx, y, dscale, lt, seed) {
    const s = seed * 0.61;
    ...
    if (mottle > 0.9)
        color = colorLighten(color, 0.08);
    else if (mottle < -0.9)
        color = colorDarken(color, 0.12);
 ...
 */
function weave(base, dx, y, dscale, lt, seed) {
 ...
 */
function verdigris(base, dx, y, dscale, lt, seed) {
    ...
    // Fine speckle.
    if (hash2(dx, y, seed + 13) > 0.94)
        color = colorMix(color, base.spec, 0.4);
    return color;
}
/**
 ...
 */
function crystal(base, dx, y, dscale, lt, seed) {
    ...
    if (Math.min(ea, eb) < 0.09)
        color = colorDarken(color, 0.32);
    // Diagonal glint band.
    const glint = Math.abs(((dx * 0.5 - y * 0.5 + seed) % (cell * 3)) / (cell * 3) - 0.5);
    if (glint < 0.06 && bright > 0.35)
        color = colorMix(color, base.spec, 0.55);
    return color;
}
/**
...
export function drawShield(pen, parts) {
    ...
    const requestedBlazon = parts?.blazon;
    const blazon = requestedBlazon && BLAZONS.includes(requestedBlazon)
        ? requestedBlazon
    ...
    const rim = parts?.rim && RIMS.includes(parts.rim) ? parts.rim : pick(r, RIMS);
    if (rim !== "none") {
        const rimRamp = rim === "gold" ? GOLD
        ...
        const scanStamp = (n, a0, fn) => {
            for (let i = 0; i < n; i++) {
                ...
                let edge = 0;
                for (let d = 0; d < B; d += 1) {
                    if (!sample(shape, dirx * d, cyMid + diry * d, m)) {
                        edge = d;
                        break;
                    }
                }
        ...
        if (rim === "riveted" || rim === "studded") {
            ...
            scanStamp(11, r.rangeFloat(0, Math.PI / 5), (px, py, edge, a) => {
                ...
                pen.fillCone(cx + dirx * (edge - baseInset), cyMid + diry * (edge - baseInset), dirx, diry, 0, Math.max(3, 3.4 * dscale), Math.max(1.4, 1.6 * dscale), STEEL.light, STEEL.shadow);
            });
        }
        else if (rim === "corners") {
    ...
    // -- centrepiece emblem -----------------------------------------------------
    const emblem = parts?.emblem && EMBLEMS.includes(parts.emblem) ? parts.emblem : pick(r, EMBLEMS);
...
function drawEmblemCrescent(pen, c, er, ramp, r) {
    ...
    for (let x = Math.floor(c.x - rad); x <= Math.ceil(c.x + rad); x++) {
        for (let y = Math.floor(c.y - rad); y <= Math.ceil(c.y + rad); y++) {
            ...
            if (Math.hypot(x - (c.x + side * off), y - c.y) < rad * 0.82)
                continue;
...
function drawBolt(pen, c, er, ramp) {
    const h = er * 0.9;
    const w = er * 0.42;
...
function drawSun(pen, c, er, ramp) {
