...
export function pickParticleType(rng) {
 ...
 */
export function pickThemedParticle(pen, rng) {
    ...
    if (sat < 42)
        return mx < 95 ? "ash" : rng.float() < 0.5 ? "spark" : "frost"; // steel / dark
    if (r > g + 20 && r > b + 20)
        return rng.float() < 0.5 ? "ember" : "blood"; // red
    if (g >= r && g > b + 15)
        return "leaf"; // green
    if (b >= r && b >= g)
        return rng.float() < 0.5 ? "frost" : "bubble"; // blue
    if (r > b + 20 && g > b)
        return rng.float() < 0.5 ? "holy" : "ember"; // gold / yellow
    return "sparkle"; // purple / other
}
export function drawParticles(pen, type, rng, count = 0) {
    const w = pen.dimension;
    const h = pen.dimension;
    ...
    for (let i = 0; i < n; i++) {
        ...
        switch (type) {
            case "sparkle":
                sparkle(plot, x, y, 1 + rng.range(0, 2));
                break;
            case "ember": {
                y -= rng.floatLow() * 4;
                const t = rng.float();
                ...
                if (rng.float() < 0.5)
                    plot(x, y - 1, 255, 90, 20, 0.5);
                break;
            }
            case "frost":
                plus(plot, x, y, 210, 240, 255, rng.float() < 0.5 ? 1 : 2, 0.85);
                break;
            case "spark":
                zig(plot, x, y);
                break;
            case "mote":
                plot(x, y, 225, 225, 235, 0.32 + rng.float() * 0.35);
                break;
            case "leaf": {
                plot(x, y, 96, 180, 72, 0.9);
                plot(x + 1, y, 60, 138, 50, 0.85);
                break;
            }
            case "bubble":
                ring(plot, x, y, 1 + rng.range(0, 1), 150, 220, 255, 0.7);
                break;
            case "blood": {
                y += rng.floatLow() * 5;
                plot(x, y, 184, 32, 42, 0.95);
                if (rng.float() < 0.5)
                    plot(x, y + 1, 120, 18, 26, 0.8);
                break;
            }
            case "holy":
                star(plot, x, y, 255, 240, 180);
                break;
            case "ash": {
                y -= rng.floatLow() * 3;
                plot(x, y, 120, 120, 132, 0.28 + rng.float() * 0.32);
                break;
            }
        }
    }
}
function sparkle(plot, cx, cy, size) {
...
function plus(plot, cx, cy, r, g, b, size, a) {
