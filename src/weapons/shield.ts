...
function blazonRamp(blazon: Blazon, nx: number, ny: number, a: Ramp, b: Ramp): Ramp {
  switch (blazon) {
    case "per-pale":
      return nx < 0 ? a : b;
    case "per-bend":
...
function applyWoodGrain(color: Color, dx: number, dscale: number): Color {
  ...
  for (let x = 0; x < B; x++) {
    for (let y = 0; y < B; y++) {
      ...
      if (inRim) {
        ...
        if (isWoodPlain) color = applyWoodGrain(color, dx, dscale);
      }
...
function paintMaskedBar(pen: Pen, x0: number, y0: number, dx: number, dy: number, len: number, half: number, ramp: Ramp): void {
  ...
  for (let s = 0; s <= steps; s++) {
    ...
    for (let w = -half; w <= half; w += 0.5) {
      ...
      if (px < 0 || py < 0 || px >= pen.dimension || py >= pen.dimension) continue;
      if (pen.ctx.getImageData(px, py, 1, 1).data[3] === 0) continue;
